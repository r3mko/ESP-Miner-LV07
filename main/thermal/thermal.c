...
esp_err_t Thermal_init(DeviceConfig * DEVICE_CONFIG)
{
    ...
    if (DEVICE_CONFIG->EMC2302_LV07) {
        ESP_LOGI(TAG, "Initializing EMC2302_LV07 and TMP1075_LV07s (Temperature offset: %d °C)", DEVICE_CONFIG->temp_offset);
        esp_err_t res_emc2302_LV07 = EMC2302_LV07_init();
        esp_err_t res_tmp1075_A    = TMP1075_LV07_init(&sensor_A, DEVICE_CONFIG->TMP1075_A, "TMP1075_A", DEVICE_CONFIG->temp_offset);
        esp_err_t res_tmp1075_B    = TMP1075_LV07_init(&sensor_B, DEVICE_CONFIG->TMP1075_B, "TMP1075_B", DEVICE_CONFIG->temp_offset);
...
esp_err_t Thermal_set_fan_percent(DeviceConfig * DEVICE_CONFIG, float percent)
{
    ...
    if (DEVICE_CONFIG->EMC2302_LV07) {
        esp_err_t res_fan_1 = EMC2302_LV07_set_fan_speed(0, percent);
        esp_err_t res_fan_2 = EMC2302_LV07_set_fan_speed(1, percent);

        // return the first non-ESP_OK
        if (res_fan_1 != ESP_OK) return res_fan_1;
        if (res_fan_2 != ESP_OK) return res_fan_2;
    }
...
uint16_t Thermal_get_fan2_speed(DeviceConfig * DEVICE_CONFIG) 
{
    ...
    if (DEVICE_CONFIG->EMC2302_LV07) {
        return EMC2302_LV07_get_fan_speed(1);
    }
    return 0;
}
...
float Thermal_get_chip_temp(GlobalState * GLOBAL_STATE)
{
    ...
    if (GLOBAL_STATE->DEVICE_CONFIG.TMP1075_A) {
...
float Thermal_get_chip_temp2(GlobalState * GLOBAL_STATE)
{
    ...
    if (GLOBAL_STATE->DEVICE_CONFIG.TMP1075_B) {
